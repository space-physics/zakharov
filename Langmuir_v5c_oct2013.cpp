...
char outDir[16]="test17"; //directory to put all results
...
vector<double> nbeam {(40.0e-7)*n0, (45.0e-7)*n0, (50.0e-7)*n0, (55.0e-7)*n0};
int Nnbeam=nbeam.size();
vector<double> vbeam_ev {40.0, 56.0, 78.0, 110.0, 154.0, 215.0, 301.0, 422.0, 590.0};
...
const double endTime=50.0e-3;    // simulation ends (seconds)
const double Tstep=1.0e-7;     // simulation time steps
double TT=endTime/Tstep;              //floor(endTime/Tstep)+2;
const int res=10;
const int TT_res=floor(endTime/Tstep/res);			//floor(endTime/Tstep/res);
double L=100.0;           // simulation box length (meter)j
const int N=1422;          // number of samples in L; should be devidable by 6
double Xstep=L/N;
