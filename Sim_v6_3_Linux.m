...
newpath='hc/test16';    useSinglePrec = false;
end
...
for Nvbeam=0:0
    close all;
    instantiation = [newpath,': n',int2str(Nnbeam),'_v',int2str(Nvbeam)];
    
    if doWriteResults
     mkdir([newpath,filesep, 'results'], ['n' num2str(Nnbeam) '_v' num2str(Nvbeam)]);
    end
%fclose('all');

fnParam = [newpath,filesep,'parameters_n' num2str(Nnbeam) '_v' num2str(Nvbeam) '.bin'];
params = Langmuir_read_params(fnParam);

fnOutput = [newpath,filesep,'output1_n' num2str(Nnbeam) '_v' num2str(Nvbeam) '.bin'];
...
colorbar;
if doWriteResults
 saveas(gcf,[newpath,filesep,'results',filesep,'n' num2str(Nnbeam) '_v' num2str(Nvbeam),filesep, 'E_kt'],'fig');
end
figure;
...
colorbar;
if doWriteResults
 saveas(gcf,[newpath,filesep,'results',filesep,'n' num2str(Nnbeam) '_v' num2str(Nvbeam),filesep,'n_kt'],'fig');
end
...
if doWriteResults
 saveas(gcf,[newpath,filesep,'results',filesep,'n' num2str(Nnbeam) '_v' num2str(Nvbeam),filesep,'regiems'],'fig');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%   average over realizations to calculate the spectrum for the last 0.5ms, for all k
    ...
    ACF_length = floor(Taverage/(Tstep*res));
...
%floor(ii/QW*100)
if ~mod(ii,1), display(['ii/QW: ',num2str(ii/QW*100,'%0.3f'),'% complete']),end
end
ACF_E=ACF_E/QW/(ACF_length-1)*2;
ACF_n=ACF_n/QW/(ACF_length-1)*2;
...
faxis=linspace(-1/(res*Tstep)/2,1/(res*Tstep)/2,ACF_length*2-1);
TT_res=length(taxis);
% TT_res=TT_res-ACF_length*30;
for oo=1:OO
...
if doWriteResults
 ...
 saveas(gcf,[newpath,filesep,'results',filesep,'n' num2str(Nnbeam) '_v' num2str(Nvbeam),filesep, nameWr],'fig');
end
